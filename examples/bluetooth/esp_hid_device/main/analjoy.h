...
#define ADC_PINX ADC_CHANNEL_5
#define ADC_PINY ADC_CHANNEL_4
...
static unsigned short read_anal(const anal_type t) {
    int adc_value;
    auto pin = t == JOY_X_AXIS ? ADC_PINX : ADC_PINY;

    // Read ADC value with Oneshot
    ESP_ERROR_CHECK(adc_oneshot_read(adc_handle, pin, &adc_value));
    // Print ADC value
